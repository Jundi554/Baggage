...
export default function App() {
  ...
  const handleParse = async (text: string) => {
    ...
    if (uniqueNewEvents.length > 0) {
      setEvents(prev => {
        // Double check against latest state just to be completely safe
      ...
      if (uniqueNewEvents[0].departureDate) {
        // Auto-navigate calendar to the parsed month if first date exists
      ...
      <header className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-2">
        <div className="flex justify-between items-center bg-white/5 backdrop-blur-xl border border-white/10 p-4 rounded-2xl">
          ...
          <div className="flex gap-4">
            {isAuthenticated ? (
              <div className="flex items-center gap-3">
                <span className="text-xs text-slate-300">Signed in as <span className="font-bold text-white">{user?.displayName || 'User'}</span></span>
                <button 
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-white/10 rounded-full border border-white/10">
              ...
              <span className="text-xs font-medium uppercase tracking-wider text-slate-200">System Active</span>
            </div>
          </div>
        </div>
      </header>
